...
def connectStr(alchemy=False):
    if alchemy:
        return f'postgresql+psycopg://postgres:{os.environ["POSTGRES_PASSWORD"]}@postgres:5432/ragtest'
...
def renderdoc(baseurl, d):
    meta = d.metadata
    if "source" in meta and "title" in meta:
        source = meta["source"]
        title = meta["title"]
        url = (
            source
            if source.startswith("http")
            else f'{baseurl}/servicedesk/customer/portal/6/article/{meta["source"]}'
        )
        return f"* [{title}]({url})"


def docsource(d):
    return d.metadata["source"] if "source" in d.metadata else None
        ...
        async for chunk in llm_chain.astream(
            ...
            if "context" in chunk:
                for d in chunk["context"]:
                    if not docsource(d) in seen:
                        seen.add(docsource(d))
                        el = renderdoc(baseurl, d)
                        if el != None:
                            elements.append(el)

        if len(elements) > 0:
            elements: List[cl.ElementBased] = [
                cl.Text("For reference see:", content="\n".join(elements))
            ]
